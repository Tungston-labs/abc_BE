...
class LCOCustomerReportView(APIView):
    ...
    def post(self, request):
        ...
        # 🚫 Prevent LCO override
        filters.pop("lco", None)

        # 🔍 Search
        search_term = request.data.get("search", "").strip()

        # ✅ BASE QUERY (LCO RESTRICTED)
        queryset = Customer.objects.filter(lco=lco).order_by("id")
        ...
        queryset = queryset.values(*resolved_fields)

        # 🛑 HARD LIMIT (avoid DB crash)
        queryset = queryset[:50000]

        # 📦 Use iterator (safe memory usage)
